...
0. 準備: メモリディレクトリ作成 → 05_log.md初期化 → **Blueprint WUのCold-Start Brief読込（あれば）** → **過去知見検索（`learnings-researcher` が利用可能なら起動。未ロードなら `rg`/SQLite または `explorer` で代替。CRITICAL・必須）**
1. 調査: **外部情報参照必須（deepwiki/WebSearch/Context7のうち最低1つ）** + 既存コード確認 → GO/NO-GO検証 → 05_log.mdに記録（コードベース調査で未知の技術要素が判明した場合のみ過去知見検索を追加実行（`learnings-researcher` が利用可能なら起動。未ロードなら `rg`/SQLite または `explorer` で代替））
...
3. 実装: 各タスクを調査→計画→実行→レビュー。**非自明なタスクでは過去知見検索を並列実行（`learnings-researcher` が利用可能なら起動。未ロードなら `rg`/SQLite または `explorer` で代替）**。こまめにコミット → 05_log.mdに記録
...
| 条件 | Codex `spawn_agent.agent_type` |
|------|------------------------------|
| 3ファイル以上の調査/レビュー | `explorer` で並列検索 |
| 複数観点からの分析 | `arch-reviewer` / `security-reviewer` / `perf-reviewer` + 変更に応じた専門レビューアーを並列 |
| 3つ以上の独立した実装・作業タスク | `worker` を並列 |
| 該当する専門ロールがない汎用タスク | `default` |

**IMPORTANT**: 独立したタスクは積極的に並列化する。Claude Task 由来の `Explore` / `general-purpose` は Codex `spawn_agent.agent_type` ではないため指定しない。`learnings-researcher` は `~/.codex/agents/` に定義済みだが、このセッションの tool schema に未ロードなら `rg`/SQLite または `explorer` で代替する。

### Codex `spawn_agent` の model/service_tier

現行 Codex `spawn_agent` では、通常 `model` と `service_tier` を省略し、親スレッドのモデル継承と各 agent role の既定値に任せる。`service_tier` だけを指定すると、モデル解決経路によって `spawn_agent could not resolve the child model for service tier validation` になることがあるため避ける。

- 通常の調査・レビュー・小規模実装: `model` / `service_tier` を省略
- 明示的な昇格が必要な場合のみ、`model: gpt-5.5`, `service_tier: priority` のように有効なペアで指定
...
- **Phase 0の過去知見検索/`deepening-plan`（3ファイル以上時）/外部情報参照をスキップすること（上記Phase 0-2の必須項目）**
