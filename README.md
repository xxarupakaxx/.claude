...
PRのCIステータスとレビューコメントを1サイクル点検します。

```bash
/pr-watch 123
/loop 30m /pr-watch 123
```

- authorのPRのみ自動修正・push
- Draft PR中はCIのみ監視し、レビュー対応はReady for review後に実行
- `git push --force` / `--force-with-lease` は使わない

---

### /team-run

Agent Teamを編成し、複数ターンに渡る協調タスクを実行します。

```bash
/team-run "フルスタック機能を実装"
```

- PJの `.claude/context/team-run.md` があれば最初に読み込む
- planner / implementer / reviewer を役割分担して進める
- PR作成後は `/pr-watch` でCI/レビュー継続監視に接続する
...
- 必要に応じて `.claude/context/team-run.md`（通知先、編成、レビュー観点）
...
| `/pr-watch` | PRのCI/レビューを1サイクル点検 | `[PR番号]`: 対象PR |
| `/team-run` | Agent Team編成 | `"<タスク説明>"`: 実行タスク |
...
新規プロジェクトでは `/project-init` を実行、または `templates/project-setup/.claude/` をプロジェクトの `.claude/` にコピー:
