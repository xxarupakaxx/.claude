...
# ---- ログディレクトリ（必要時のみ）----
LOG_DIR="$HOME/.Codex/.local/hooks/log"
...
| `2` | **ブロック**（PreToolUse: ツール実行阻止 / Stop: Codex を再起動して継続） |

Stop / SubagentStop で `{"decision":"block","reason":"..."}` を stdout に出すと、reason が新しいユーザー指示として Codex に返る。
    ...
    "PostToolUse": [
      {
        "matcher": "Edit|Write",
        "hooks": [
          { "type": "command", "command": "$CLAUDE_PROJECT_DIR/.Codex/hooks/<name>.sh" }
        ]
      }
    ]
  }
...
ユーザー設定の場合は `~/.Codex/hooks/<name>.sh` の絶対パスで登録。

### Step 7: 実行権限付与

```bash
chmod +x ~/.Codex/hooks/<name>.sh
```
...
```bash
LOG_FILE="$HOME/.Codex/.local/hooks/log/<name>.jsonl"
...
- **無限ループ防止抜け**: Stop/SubagentStop で `stop_hook_active` を見ない → Codex が永久ループ
...
- [ ] ログ出力先は `~/.Codex/.local/hooks/log/` 配下
...
- 既存例:
  - `~/.Codex/hooks/pre-dangerous-command-block.sh`（PreToolUse ブロック例）
  - `~/.Codex/hooks/stop-workflow-check.sh`（Stop + block 例）
  - `~/.Codex/hooks/post-auto-format.sh`（PostToolUse 例）
  - `~/.Codex/hooks/session-start-inject.sh`（SessionStart 例）
