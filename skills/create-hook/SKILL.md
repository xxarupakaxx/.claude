...
| `2` | **ブロック**（PreToolUse: ツール実行阻止 / Stop: Claude を再起動して継続） |

Stop / SubagentStop で `{"decision":"block","reason":"..."}` を stdout に出すと、reason が新しいユーザー指示として Claude に返る。
...
- **無限ループ防止抜け**: Stop/SubagentStop で `stop_hook_active` を見ない → Claude が永久ループ
