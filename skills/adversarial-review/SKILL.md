...
Codex では原則 `model` を指定せず、親スレッドのモデルを継承する。
明示指定が必要な場合だけ、起動側で以下を使う:

| エージェント | model override | 役割 |
|------------|-------|------|
| `red-reviewer` | 原則未指定 | 攻撃側（広く速く懸念列挙） |
| `blue-reviewer` | 原則未指定 | 防御側（Red の反論検証） |
| `auditor-reviewer` | 必要時のみ `gpt-5.5` | 審判（最終判定・Read で独立検証） |
...
- Red: 親モデル継承
- Blue: 親モデル継承
- Auditor: 親モデル継承、必要時のみ `gpt-5.5`
- **方針**: 全員に高精度モデルを明示指定せず、判断が重い箇所だけ昇格する
