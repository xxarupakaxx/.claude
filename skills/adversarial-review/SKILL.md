...
現行 Codex `spawn_agent` では `model` と `service_tier` をペアで明示する:

| エージェント | spawn時の指定 | 役割 |
|------------|-------|------|
| `red-reviewer` | `model: gpt-5.4`, `service_tier: priority` | 攻撃側（広く速く懸念列挙） |
| `blue-reviewer` | `model: gpt-5.4`, `service_tier: priority` | 防御側（Red の反論検証） |
| `auditor-reviewer` | `model: gpt-5.5`, `service_tier: priority` | 審判（最終判定・Read で独立検証） |
...
- Red: `gpt-5.4` + `priority`
- Blue: `gpt-5.4` + `priority`
- Auditor: `gpt-5.5` + `priority`
- **方針**: `service_tier` 未指定経路を避けつつ、判断が重い箇所だけ `gpt-5.5` に昇格する
