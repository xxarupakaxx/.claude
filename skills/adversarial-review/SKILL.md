...
現行 Codex `spawn_agent` では通常 `model` と `service_tier` を省略し、各 role の既定値に任せる:

| エージェント | agent_type / 既定 | 役割 |
|------------|-------|------|
| `red-reviewer` | spawn時は `agent_type: red-reviewer` のみ（role既定: `gpt-5.4` + `priority`） | 攻撃側（広く速く懸念列挙） |
| `blue-reviewer` | spawn時は `agent_type: blue-reviewer` のみ（role既定: `gpt-5.4` + `priority`） | 防御側（Red の反論検証） |
| `auditor-reviewer` | spawn時は `agent_type: auditor-reviewer` のみ（role既定: `gpt-5.5` + `priority`） | 審判（最終判定・Read で独立検証） |

> `sonnet` / `opus` / `haiku` は Codex の `spawn_agent` model override として指定しない。
...
- Red: `agent_type: red-reviewer`（role既定: `gpt-5.4` + `priority`）
- Blue: `agent_type: blue-reviewer`（role既定: `gpt-5.4` + `priority`）
- Auditor: `agent_type: auditor-reviewer`（role既定: `gpt-5.5` + `priority`）
- **方針**: spawn時の `model` / `service_tier` は通常省略し、`service_tier` 単独指定を避ける
