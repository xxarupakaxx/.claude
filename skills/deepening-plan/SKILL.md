...
description: |
  ...
  Phase 2（計画）で30_plan.html作成後、追加調査で意思決定が変わる不確実性がある場合だけ使用する。
  「計画を深掘りして」「deepen planして」「計画をリサーチで補強して」等の依頼にも対応。
  既知手順や、追加調査で計画が変わらない場合は省略する。
---

# Deepening Plan

既存の計画（30_plan.html）の各セクションに対して並列リサーチを実行し、
具体性・正確性・網羅性を強化する。
...
- **入力**: 30_plan.html（実装計画）、solutions/の過去知見
- **出力**: 強化された30_plan.html（リサーチ結果・リスク・参考情報を追記）
- **後続**: サブエージェントレビューが計画の品質を検証

## 前提条件

- 30_plan.html（実装計画）が作成済みであること。HTMLがない既存taskでは30_plan.mdを分析用の互換入力として読む。
- 計画にタスクが定義されていること
...
30_plan.htmlを読み取り、各タスクの以下を特定する。HTMLがない既存taskでは30_plan.mdを分析用の互換入力として読む。
- 使用するライブラリ/フレームワーク
- 変更対象の技術領域
- 不確実性が高いポイント
- 外部依存（API, DB, 外部サービス等）
...
リサーチ結果を元に30_plan.htmlを更新。既存Markdownを更新・変換して新しい正本にしない:
