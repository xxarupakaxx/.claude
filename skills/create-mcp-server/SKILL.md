...
- **~/.claude.json**: user-level MCP 登録先
...
- **schema 省略**: `inputSchema` 未指定だと Claude が呼べない
...
- **再起動忘れ**: ~/.claude.json 編集後は Claude Code 再起動が必要
...
- [ ] ~/.claude.json or .mcp.json 登録例を提示
