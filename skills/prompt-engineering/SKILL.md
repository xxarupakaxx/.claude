...
3. **構造化入出力** — スキーマ定義、XMLタグ（Codex固有）、JSON構造
...
**デフォルト前提**: Codexはすでに非常に賢い。Codexが「まだ持っていない」コンテキストのみを追加する。

## コアテクニック

1. **Few-Shot Learning** — 2-5個の入出力ペアで動作を教示
2. **Chain-of-Thought** — ステップバイステップ推論で精度30-50%向上
3. **Adaptive/Extended Thinking** — Codex 4.x: effortパラメータで思考深度を制御
4. **テンプレートシステム** — 変数・条件付きセクションの再利用可能構造
5. **XMLタグ構造化（Codex固有）** — Codex訓練データに含まれるXMLタグで曖昧さを排除。他LLMでは効果が異なる
...
- **Prefill廃止**: Codex 4.6でassistant turnのprefillは非対応。structured outputs/XML/直接指示で代替
...
AGENTS.md設計、skills/hooks設計、サブエージェントプロンプトの実践パターン。

詳細: `Read references/Codex-patterns.md`
...
- **Overtrigger**: 以前のモデル向けの強い言語がCodex 4.xで過剰反応を招く
...
- [Anthropic公式: Prompting best practices (Codex 4.x)](https://docs.anthropic.com/en/docs/build-with-Codex/prompt-engineering/Codex-4-best-practices)
