...
**CRITICAL: `Codex -p` CLIは使わない。Taskツールの専門サブエージェントを並列起動すること。**
...
| トリガー（diffに含まれる内容） | agent_type | レビュー観点 |
|---|---|---|
...
- 変更対象ファイルのフルパス一覧
- PJのAGENTS.mdのパス
- レビュー観点と出力形式（CRITICAL/IMPORTANT/MINOR 分類。AGENTS.md `severity` 標準）
...
## PJルール
[AGENTS.mdの内容を読んで参照してください: <AGENTS.mdパス>]
