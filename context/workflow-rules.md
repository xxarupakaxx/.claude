...
5. `learnings-researcher` agent_type が利用可能ならタスクに関連する過去知見を検索（memories/ + solutions/ + issues/ を横断）。未ロードなら `rg`/SQLite または `explorer` で代替し、結果を05_log.mdに記録
...
`learnings-researcher` agent_type が利用可能なら Codex `spawn_agent` で起動し、タスクに関連する過去知見を構造化検索する。現在のセッションで未ロードの場合は、`rg`/SQLite 検索をローカル実行するか `explorer` に同じ検索プロンプトを渡す。`general-purpose` は Codex の agent_type ではないため指定しない:
...
`adversarial-review` skill が起動するときに Tier 1-3 とは別経路で起動される 3 エージェント。直接呼び出しは推奨しない（必ず skill 経由）。spawn時の `model` / `service_tier` は通常省略し、各 role の既定値に任せる。

| エージェント | Codex agent_type / 既定 | 役割 |
|------------|-------|------|
| `red-reviewer` | `model: gpt-5.4`, `service_tier: priority` | 攻撃側 (Pessimist)。コードの欠陥・脆弱性・設計ミスを最大限あぶり出す。JSONL 出力 |
| `blue-reviewer` | `model: gpt-5.4`, `service_tier: priority` | 防御側 (Optimist of Skeptic)。Red 各指摘を独立検証し AGREE/PARTIAL/REJECT を付与 |
| `auditor-reviewer` | `model: gpt-5.5`, `service_tier: priority` | 審判 (Judge)。Red/Blue 不一致点を優先分析し、Read で独立検証して ADOPT/DOWNGRADE/UPGRADE/REJECT/ESCALATE を最終判定 |

**起動条件**: DBスキーマ変更、認証フロー変更、外部 API 契約変更等の重要判断、または `auto-reviewing-pre-pr` が ESCALATE を返した場合。
**コスト方針**: 現行 Codex `spawn_agent` では通常 `model` と `service_tier` を省略する。明示的な昇格が必要な場合のみ `gpt-5.5` + `priority` のように有効なペアで指定する。
