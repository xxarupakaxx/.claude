...
- 深掘り (重要判断): `adversarial-review` で Red(攻撃 sonnet) → Blue(防御 sonnet) → Auditor(審判 opus)
...
| `adversarial-review` | スキル | Red(攻撃 sonnet) → Blue(防御 sonnet) → Auditor(審判 opus) の3-agent 直列レビュー。重要判断時のみ起動（コスト 70% 削減） | 4（重要判断時） |
...
`learnings-researcher`エージェント（Taskツール、subagent_type=`learnings-researcher` または `general-purpose`）を起動し、タスクに関連する過去知見を構造化検索:
...
完全な状態遷移マトリクス（A〜F の 6 状態）は **`~/.claude/skills/auto-reviewing-pre-pr/SKILL.md` を参照**。本ファイルは概要のみ:

- 合格 (CONVERGED): CRITICAL=0 かつ IMPORTANT<3
- 継続 (CONTINUE): 残存あり、round<max、同一指摘<3
- 人間判断 (ESCALATE): 同一指摘 ≥3 ラウンド連続 / round ≥ max（小3/中5/大8）
- 中断 (ABORT): LLM単独修正連続3回失敗 → 静的解析+人間確認を強制

> 詳細条件・遷移表の二重定義を防ぐため、状態 B/C や severity 閾値の境界値は SKILL.md 側を正とする。
...
`adversarial-review` skill が起動するときに Tier 1-3 とは別経路で起動される 3 エージェント。直接呼び出しは推奨しない（必ず skill 経由）。

| エージェント | model | 役割 |
|------------|-------|------|
| `red-reviewer` | sonnet | 攻撃側 (Pessimist)。コードの欠陥・脆弱性・設計ミスを最大限あぶり出す。JSONL 出力 |
| `blue-reviewer` | sonnet | 防御側 (Optimist of Skeptic)。Red 各指摘を独立検証し AGREE/PARTIAL/REJECT を付与 |
| `auditor-reviewer` | opus | 審判 (Judge)。Red/Blue 不一致点を優先分析し、Read で独立検証して ADOPT/DOWNGRADE/UPGRADE/REJECT/ESCALATE を最終判定 |

**起動条件**: DBスキーマ変更、認証フロー変更、外部 API 契約変更等の重要判断、または `auto-reviewing-pre-pr` が ESCALATE を返した場合。
**コスト**: 1 回 ~$0.70-1.60（全 opus 構成より 70% 削減）。
