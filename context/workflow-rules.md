...
**収束判定マトリクス**（`auto-reviewing-pre-pr/SKILL.md` 参照）:
- A) CRITICAL=0 かつ IMPORTANT<3 → CONVERGED 合格
- D) 同一指摘 ≥3 ラウンド連続残存 → ESCALATE 人間判断
- E) round ≥ max（小3/中5/大8） → ESCALATE
- F) LLM単独修正連続3回失敗 → ABORT（静的解析+人間確認を強制）
