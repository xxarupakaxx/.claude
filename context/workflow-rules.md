...
|------------|-------|------|
| `red-reviewer` | 原則未指定 | 攻撃側 (Pessimist)。コードの欠陥・脆弱性・設計ミスを最大限あぶり出す。JSONL 出力 |
| `blue-reviewer` | 原則未指定 | 防御側 (Optimist of Skeptic)。Red 各指摘を独立検証し AGREE/PARTIAL/REJECT を付与 |
| `auditor-reviewer` | 必要時のみ `gpt-5.5` | 審判 (Judge)。Red/Blue 不一致点を優先分析し、Read で独立検証して ADOPT/DOWNGRADE/UPGRADE/REJECT/ESCALATE を最終判定 |

**起動条件**: DBスキーマ変更、認証フロー変更、外部 API 契約変更等の重要判断、または `auto-reviewing-pre-pr` が ESCALATE を返した場合。
**コスト方針**: 原則は親モデル継承。判断が重い Auditor のみ必要時に model override を検討する。
