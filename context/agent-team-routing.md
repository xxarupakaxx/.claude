...
- **retired**：deprecatedな `conducting-quality-assurance`、`planning-refactors`、`ubiquitous-language` はruntimeから削除済み。置換先は `triaging-issues` と `diagnosing-bugs`、architecture surveyと`to-spec`と`to-tickets`、`modeling-domains`。
