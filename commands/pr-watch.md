...
- **CI全green かつ 未対応レビューなし** → 「[完了] 全チェック通過・レビュー対応完了。これ以上の対応は不要です。`/loop` を `Esc` で停止できます」。**commit / push はしない（no-op）**
...
- 1サイクルの自動修正上限: CI修正は check 毎に2回（`ci_fix_attempts`）+ レビュー3ラウンド（pr-review-loop 側で担保）。CI全green後は no-op
