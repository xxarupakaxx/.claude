...
マルチの本当のコストは金銭・トークンではなく、**文脈分断**（teammate は lead の会話履歴を持たず、結果は要約で戻る）と**無駄撃ち**（価値を生まない起動）にある。トークンを揃えても単一が拮抗〜凌駕する報告（Stanford）のとおり、分業そのものは品質を上げない。**独立した作業幅・隔離された専門知識・独立検証のいずれかに価値があれば躊躇なく使い、無いのに同じ文脈を分業させない。** 基準は単一＋良い文脈、並列の幅が本当に要る時だけ team-run。
...
1. **L0 local**：Grep、Glob、`git diff`、既存 script で完結するなら agent を起動しない。
2. **L1 haiku**：commit 文案、短いログ要約、定型整形、重複検出だけなら、必要なときだけ `haiku` を使う。
3. **L2 sonnet**：探索、routine 実装、通常ワーカーには `sonnet` を使う。
4. **L3 opus**：計画、GO/NO-GO、重要設計、セキュリティ、専門レビューには `opus` を使う。

実際の `git add`、`git commit`、`git push` は shell で実行する。
commit に関して `haiku` へ任せられるのは、メッセージ文案の作成だけである。
不確実性、矛盾、複数ファイルにまたがる判断、ユーザー影響が出たら、その round を止めて `sonnet` または `opus` へ昇格する。
...
- ユーザーが逐語で読むべき成果物（計画・重要 findings・生成文書）は、完了を待たずファイルに書き、その path を SendMessage で lead に知らせる
```

### Context Slimming

teammate へ渡す文脈は、objective、scope、acceptance、prior failure、output の薄い束にする。
Team Journal と `05_log.md` を正本とし、同じ背景全文を round ごとに貼り直さない。
...
- teammate から path 付きで届いた user-facing 成果物（ユーザーが逐語で読むべき計画・重要 findings・生成文書）は、lead が Read して要約せず原文をチャットに提示する
...
3. **S2. Team Journal 初期化**（`.local/memory/<task>/team-journal.md`）。冒頭の「定位置」に **合格基準 / Budget / leader 状態 / model route** を書く。
...
9. **S3. 割り当て.** leader が Dispatch Table（`rules/model-routing.md`）で配分（計画=planner/opus、調査=Explore/sonnet、routine実装=sonnet、重い実装=codex、レビュー=専門 reviewer/opus）。Cost Ladder の選択理由を Team Journal に記録する。spawn prompt に objective、境界、成果物形式、**前周の失敗原因の要約を leader が直接渡す**。症状だけでは、次の round が同じ原因で別の症状を踏む。

commit 文案、短い要約、定型整形、重複検出だけは `haiku` を使ってよい。
実装、設計、最終レビューには使わない。
...
13. **S6. maker/checker（ペルソナで分業）.** codex（maker）の「できた」は checker が通すまで**未完了**:
   - **maker**=codex（test を改変しない誠実な実装を spawn で課す）/ **checker**=既存の専門 reviewer（MCP・スキル非依存のものを選ぶ）/ **Auditor**=`adversarial-review` の審判（**決裂＝同一 finding で2往復しても不合格、の時だけ**発動）
   - (a) **デフォルト不合格**（CRITICAL/IMPORTANT についてのみ）。合格は根拠を Journal に書く（立証責任の反転）
   - (b) checker に maker の自己申告を見せず、成果物だけ渡す
   - (c) arch/security/perf が各観点で見て 1つでも CRITICAL なら不合格
   - (d) checker は **test の差分も疑う**（test を緩めた/skip/削除した形跡＝不合格・reward hacking 対策）
   - **合格ライン**: 機械判定 green ＋ 全 checker の CRITICAL/IMPORTANT がゼロ。**MINOR は合格を妨げない**（完璧主義を抑制し loop を収束させる）
   - 合否が割れたら不合格優先 → 決裂が続けば Auditor が裁定 → なお決まらねば human に escalate
...
> **eval 接続点（将来）**: team-run の良し悪しを測る生データは Team Journal にある——タスク完了率 / 差し戻し回数 / 決定の手戻りを Trace・Attribution 節から集計でき pass@k の入力になる（`skills/eval-harness/SKILL.md`）。本格 eval は別タスク。
...
> **設計判断（grill で決着・design.md 参照）**: maker/checker タイブレーク=Auditor 裁定 / チーム縮退=対処不要(idle は無料) / DAG=疑わしきは順次+owner 分離+S7 統合検証で後追い。
